...
ListerFichiers()
{
    ...
    do
        ...
        then
            extension=${i##*.}
            if [ $extension == $extensionValide ] then echo $source"/"$i >> $nomFichierSortie fi
        fi
    done

    if [ $extension == $extensionValide ] then echo -e "\n" >> $nomFichierSortie fi
}
...
then
    ...
    if [ -f "$nomFichierChoixDossier" ] then rm -fr "$nomFichierChoixDossier" fi
